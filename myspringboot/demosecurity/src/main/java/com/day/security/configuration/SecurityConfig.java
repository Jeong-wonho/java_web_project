...
//	모든 인증은 AuthenticationManager를 통해 이루어지며 AuthenticationManager를 생성하기 위해서는 AuthenticationManagerBuilder를 사용
	@Autowired
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception{ //권한에 대해서 확인 하는게 configureGlobal이라고 한다./ 권한 관련 테이블을 만들어야 한다.
...
//		auth.inMemoryAuthentication().withUser("b").password(passwordEncoder().encode("2")).roles("MANAGER");
		...
		String query1 = "SELECT userid, userpw, enabled FROM tbl_member WHERE userid=?"; //username, pwd 일치 확인
		String query2 = "SELECT userid, auth FROM tbl_member_auth WHERE userid=?"; // 권한이 무엇인지 확인하는 column
...
//		                 
			.formLogin()
				.loginPage("/myLogin") // the custom login page
				                       //커스텀 로그인 form의 action 경로와 loginPage()의 파라미터 경로가 일치해야 인증을 처리할 수 있음
