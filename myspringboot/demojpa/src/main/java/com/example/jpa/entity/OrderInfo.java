...
public class OrderInfo {
	...
	@ManyToOne
	@JoinColumn(name = "order_id") //foreign key로 참여할 컬럼을 join column에 넣어주면 된다.! (중요!)
//	@Transient
	...
	@OneToMany(
			...
			, 
			//JPA는 연관관계가 있는 엔티티를 조회할 때 defualt로 지연로딩을 합니다(lazy loading).
			//여러 엔티티들과 종속적인 관계를 맺고 있다면 조인이 필요, 조인이 복잡할 수록 성능저하!, 
			//정보가 필요하기 전까지 최대한 테이블에 접근하지 않는 방식을 사용합니다.
			//즉시 로딩은 조인을 이용해서 정보를 처리합니다.(entity가 발생할때마다 insert구문이 작용한다.)
			//즉시로딩을 안한다면 @Transactional로 처리할 수 도 있습니다. => 같은 transaction들이 순서대로 배치가 된다.
