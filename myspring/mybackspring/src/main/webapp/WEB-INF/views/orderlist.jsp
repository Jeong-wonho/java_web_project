...
int status = 1;
if(statusInt !=null){
	status = statusInt.intValue();	//0인경우 => 로그인안한경우, -1인경우 => 주문목록없는경우 또는 예외발생한경우
}
if(status == 0) {
%>로그인하세요
<%}else if(status == -1){//주문목록이 없는 경우
%>${requestScope.msg}
<%} else {
	List<OrderInfo> infos = (List)request.getAttribute("infos");
%><table border ="1">
<tr>
<th>주문번호</th><th>주문일자</th><th>상품번호</th><th>상품명</th><th>가격</th><th>수량</th></tr>
